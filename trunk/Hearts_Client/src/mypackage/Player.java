...
public class Player {
	...
	public boolean isValid(Card c, boolean chatCo, boolean luotDau, Card kiemtra) {
		...
		if(isFirst) {
			...
			if(luotDau) {
				if(c.isTwoClub()) return true;
				else {
					if(this.isHuman()) Hearts.notice("Ban phai chon la 2 Chuon.");
					return false;
				}
			}
					...
					for(int i=0; i<listCard.size(); i++)
						if(listCard.get(i) != null)
							if(!listCard.get(i).isHeart()) {
								if(this.isHuman()) Hearts.notice("Chua duoc chon la bai chat Co. Hay chon la bai chat khac.");
								return false;
							}
		...
		//Neu la nguoi danh sau
			...
			if(c.getSuit() == kiemtra.getSuit()) return true;
			//Neu c khong cung chat voi kiemtra
				...
				for(int i=0; i<listCard.size(); i++)
					if(listCard.get(i) != null)
						if(listCard.get(i).getSuit() == kiemtra.getSuit()) {
							if(this.isHuman()) Hearts.notice("Ban phai chon 1 la bai chat "+ Card.suitText[kiemtra.getSuit()] + ".");
							return false;
						}
				//Neu tren tay khong co quan nao cung chat voi kiemtra	
				//	Neu c khong phai la quan chat co hoac khong phai la Q bich: hop le
				if(!c.isHeart() && !c.isQueenSpade()) return true;
					...
					for(int i=0; i<listCard.size(); i++)
						if(listCard.get(i) != null)
							if(!listCard.get(i).isHeart()) {
								//Neu o luot dau
								if(luotDau) {
									if(this.isHuman()) Hearts.notice("Ban khong duoc chon chat Co o luot dau tien.");
									return false;
								}
								else {
									return true;
				...
				if(c.isQueenSpade()) {
					if(luotDau){
						...
						for(int i=0; i<listCard.size(); i++)
							if(listCard.get(i) != null)
								if(!listCard.get(i).isHeart()) return true;
						//Neu toan chat co 
						if(this.isHuman()) Hearts.notice("Ban khong duoc chon la Q Bich o luot dau tien.");
						return false;
					}
					else return true;
				}			
	...
	public void addAScoreCard(Card arg0) {//, Card arg1, Card arg2, Card arg3) {
		if(arg0.isHeart() || arg0.isQueenSpade()) {
			socreCard.add(arg0);
			if(arg0.isHeart()) score+=1;
			if(arg0.isQueenSpade()) score+=13;
		}
	}
	...
	public boolean isShootTheMoon() {
		if(socreCard.size() == SOQUANBAI + 1) return true;
		return false; 
	}
	...
	public void showPlayCardLabel(int i, Card c) {
		if(i<0 || i>playCardLabel.length) return;
	...
	public void showListScoreCard() {
		if(socreCard == null || socreCard.size()==0) return; 
